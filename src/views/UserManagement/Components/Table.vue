<template>
  <div>
    ...
    >
      <v-responsive>
        <v-data-table :headers="headers" :items="Users" items-per-page="100">
          <template v-slot:top>
            <v-toolbar flat>
              <v-toolbar-title
                ><center>
                  <span class="table_topic">All Users</span>
          ...
          >
            ...
            <div v-if="header.key === 'name'">
              {{ firstLetterUpperCase(props.item.name) }}
            </div>
            ...
            <div v-if="header.key === 'contact_no'">
              <span> {{ props.item.contact_no }}</span>
            </div>

            <!-- address -->
            <div v-if="header.key === 'address'">
              <span>
                {{ firstLetterUpperCase(props.item.address) }}
              </span>
            </div>

            <!-- user type -->
            <div v-if="header.key === 'user_type'">
              <span>
                {{ props.item.user_with_roles[0].name }}
              </span>
            </div>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { title: "Mobile", align: "start", key: "contact_no" },
        { title: "Address", align: "start", key: "address" },
        { title: "Role", align: "start", key: "user_type" },
