      ...
      <v-row>
        ...
        <v-col lg="9" class="text-right" cols="12">
          <v-btn class="create_btn" variant="none" @click="show = true">
            ...
            <span class="text">Create ShowRoom</span>
          </v-btn>
        </v-col>
      </v-row>
...
export default {
  ...
  async created() {
    await this.getAllDistributers();
  },

  methods: {
    // get all  Distributers
    async getAllDistributers() {
      this.loading = true;
      ...
      const res = await DistributerApi.allDistributers(payload);
      const pagination = res.data.data;

      this.Distributers = pagination.data;
