<template>
  <div class="mt-9 masterdata">
    <div class="pt-12">
      <v-row>
        ...
        <v-col lg="6" class="text-right" cols="12">
          <v-btn class="create_btn" variant="none" @click="show = true">
            ...
            <span class="text">Create SubDistributor</span>
          </v-btn>
        </v-col>
      </v-row>
      ...
      <v-card>
        <Table
          :SubDistributers="SubDistributers"
          :loading="loading"
          @close="closeModal"
        />
      </v-card>
...
export default {
  ...
  async created() {
    await this.getAllSubDistributers();
  },

  methods: {
    // get all  SubDistributer
    async getAllSubDistributers() {
      this.loading = true;
